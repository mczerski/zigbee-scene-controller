...
void off_state_led(uint8_t count)
{
    ...
    ZB_SCHEDULE_APP_ALARM_CANCEL(off_state_led, ZB_ALARM_ANY_PARAM);
    led_off(led_dev, led_idx);
    if (count > 1) {
        ZB_SCHEDULE_APP_ALARM(
            do_blink_state_led,
            count == LED_BLINK_FOREVER ? LED_BLINK_FOREVER : count - 1,
            ZB_MILLISECONDS_TO_BEACON_INTERVAL(off_time)
        );
    }
}
